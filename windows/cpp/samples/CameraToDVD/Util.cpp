...
primo::codecs::ColorFormat::Enum GetColorFormat(GUID& videoSubtype)
{
	...
	static ColorSpaceEntry ColorSpaceTab[] = {
		...
		{ MEDIASUBTYPE_ARGB32,		ColorFormat::BGR32		}, // with alpha
		{ MEDIASUBTYPE_RGB32,		ColorFormat::BGR32		}, // with alpha
...
bool CheckRecorderDevices(primo::dvdbuilder::VR::VideoRecorder* pRecorder, int* pActiveCount, int* pFailedCount, int* pNoSpaceCount)
{
	...
	for (int i = 0; i < pRecorder->devices()->count(); i++)
	{
		ErrorInfo* pErr = pRecorder->deviceError(i);
		if (pErr->facility() != ErrorFacility::Success)
		{
			if (pErr->facility() == ErrorFacility::VRDevice &&
				pErr->code() == VRDeviceError::OutOfFreeSpace)
			{
				++noSpaceCount;
			}
			else
			...
			ATLTRACE2(L"Device(%d) error:%d facility:%d sysdomain:%d", i, pErr->code(), pErr->facility());
		}
		else
		{
			++activeCount;
	...
	if (pActiveCount) *pActiveCount = activeCount;
	if (pFailedCount) *pFailedCount = failedCount;
	if (pNoSpaceCount) *pNoSpaceCount = noSpaceCount;

	ATLTRACE2(L"Devices active:%d failed:%d out-of-space:%d", activeCount, failedCount, noSpaceCount);
